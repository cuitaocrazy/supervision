...
class TransactionService {
  ...
  async query(req) {
    ...
    const contracts = await mysql
      ...
      .where("transaction.contractId like :contractId  ", {
        contractId: nullableFuzzy(contractId),
      })
      // .skip(page*size)
      //todo 方便测试
  ...
  async balanceQuery(req) {
    ...
    var where =
      ...
      " tran_Date like :tranDate and transaction.tran_Type != 'cancel' ";
    const amount = await mysql
      .getRepository(Transaction)
      .createQueryBuilder("transaction")
      .select(
        "SUM(transaction.transactionAmt) sum,edu_supervised_account account"
      )
      .where(where, {
        account: account,
        tranDate: nullableFuzzy(tranDate),
        number: account == null ? 1 : 2,
      })
    ...
    if (amount == null) {
      return {
        result: true,
        records: [{ sum: 0, tranDate: tranDate, account: "" }],
      };
    }
    return {
      result: true,
      records: [
        { sum: amount.sum, tranDate: tranDate, account: amount.account },
      ],
    };
    // .getRawOne()
  }
  ...
  async sum(date) {
    ...
    var where = " transaction.tran_Type != 'cancel' and tran_date like :date ";
    const infos = await mysql
      ...
      .where(where, { date: nullableFuzzy(date) })
