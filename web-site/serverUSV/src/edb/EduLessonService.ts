...
class EduLessonService {
  async find(req) {
    ...
    const eduLessons = await mysql
      ...
      .where(
        "eduLesson.eduName like :name and eduLesson.lessonStatus like :status",
        { name: nullableFuzzy(eduName), status: nullableFuzzy(status) }
      )
  ...
  async findByLessonName(lessonName) {
    const eduLessons = await mysql
      .getRepository(EduLesson)
      .createQueryBuilder("eduLesson")
