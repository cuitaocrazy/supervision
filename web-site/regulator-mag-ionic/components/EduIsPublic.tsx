...
const IsOrNotPublic = [
  { id: 1, name: '是', unavailable: true },
  { id: 2, name: '否', unavailable: false },
]

// 教育机构是否公办下拉菜单组件
const EduIsPublic = () => {
  const [isPublic, setIsPublic] = useState(IsOrNotPublic[0])
  return (
    <div className="w-64 ">
      <Listbox value={isPublic} onChange={setIsPublic} >
        <div className="relative h-8">
          <Listbox.Button className="relative w-full h-10 py-1 pl-2 pr-10 text-left bg-white border rounded-md shadow-md cursor-default focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
            <span className="block truncate">{isPublic.name}</span>
          ...
          >
            <Listbox.Options className="absolute w-full py-1 mt-1 overflow-auto text-base bg-white rounded-md shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              {IsOrNotPublic.map((isOrNotPublic, isOrNotPublicIdx) => (
                <Listbox.Option
                  key={isOrNotPublicIdx}
                  className={({ active }) =>
                    `relative cursor-default select-none py-1 pl-10 pr-4 ${active ? 'bg-primary-100 text-primary-900' : 'text-gray-900'
                    }`
                  }
                  value={isOrNotPublic}
                >
