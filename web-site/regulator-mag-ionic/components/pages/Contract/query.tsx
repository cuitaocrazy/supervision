...
const ContractQuery: React.FC = () => {
  ...
  const onQuery = () => {
    fetch(paramStr, {
      ...
      .then(json => {
        ...
        refreshList(
          demoContractList
            .filter((contract: Contract) => contract.eduName.indexOf(queryInfo.eduName) > -1)
            .filter(
              (contract: Contract) => contract.consumerName.indexOf(queryInfo.consumerName) > -1
            )
            .filter((contract: Contract) => contract.lessonName.indexOf(queryInfo.lessonName) > -1)
        );
      });
  };
  ...
  const ListEntry = ({ contract, myKey, ...props }: { contract: Contract; myKey: any }) => (
    ...
    >
      <td className="flex items-center justify-center leading-10">{contract.eduId}</td>
    ...
    //   <IonLabel>
    //     <div className="flex gap-2">
    //       <button className="p-1 text-white bg-blue-500 rounded-md" onClick={onDetail(contract)}>
    //         详情
    //       </button>
    //     </div>
    //   </IonLabel>
