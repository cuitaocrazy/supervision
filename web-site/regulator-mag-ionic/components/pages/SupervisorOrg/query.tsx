...
import {
  AppContext,
  setSupervisorOrgEdit,
  setSupervisorOrgList,
  setSupervisorOrgDetail,
} from '../../../appState';
...
const LessonQuery: React.FC = () => {
  ...
  const refreshSuperviseList = useCallback(
    (orgs: SupervisorOrg[]) => {
  ...
  const doSetDetail = useCallback(
    (userInfo: SupervisorOrg | undefined) => {
      dispatch({
        ...setSupervisorOrgDetail(userInfo),
        ...{ backPage: '/tabs/supervisorOrg/query' },
      });
    },
    [dispatch]
  );
  ...
  const onQuery = () => {
    refreshSuperviseList(
      demoLessonList.filter(
        (item: SupervisorOrg) => item.supervisorOrgName.indexOf(queryInfo.supervisorOrgName) > -1
      )
    );
  };
    ...
    <IonItem key={key}>
      ...
      <IonLabel>
        <div className="flex gap-2">
          <button className="p-1 text-white bg-blue-500 rounded-md" onClick={onDetail(item)}>
            查看详情
          </button>
          <button className="p-1 text-white bg-blue-500 rounded-md" onClick={onEdit(item)}>
            编辑
          </button>
          <button className="p-1 text-white bg-blue-500 rounded-md" onClick={onCancel(item)}>
            删除
          </button>
        </div>
      </IonLabel>
          ...
          <IonRow className="flex justify-between ">
            <IonCol className="flex ml-8">
              ...
              <input
                ...
                onChange={e =>
                  setQueryInfo({ ...queryInfo, ...{ parentSupervisorOrgName: e.target.value } })
                }
              />
            </IonCol>
            <IonCol className="flex ml-8">
            ...
            <form onSubmit={onHandle}>
              <IonList>
                <IonItem>
                  ...
                  <IonInput
                    name="eduName"
                    value={newOrgState.supervisorOrgName}
                    onIonChange={e =>
                      setNewOrgState({ ...newOrgState, supervisorOrgName: e.detail.value! })
                    }
                  ></IonInput>
                </IonItem>
              </IonList>
