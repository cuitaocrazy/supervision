  ...
  <form onSubmit={handleSubmit(onSubmit)} className="p-12 px-6 py-10 pt-4 mx-auto my-auto border-2 rounded-lg shadow-md border-primaryColor-300 w-80 bg-primaryColor-100">
    <fieldset >
      ...
      <div>
        ...
        <input type="text" className="block w-full px-1 py-3 bg-white border-2 rounded-md border-primaryColor-200 focus:ring-opacity-20 focus:border-secondaryColor-500 focus:ring-offset-blue-300 focus:ring-4"
          {...register('username', { required: true },)} placeholder="请输入用户名" />
        {errors.username && <p className="pt-2 pl-3 text-base text-errorColor-400">用户名不能为空</p>}
      </div>
      <div className="mt-6">
        <label className="block mb-2 text-sm font-semibold text-gray-600">密码</label>
        <input type="text" className="block w-full px-1 py-3 bg-white border-2 rounded-md border-primaryColor-200 focus:ring-opacity-20 focus:border-secondaryColor-500 focus:ring-offset-blue-300 focus:ring-4"
          {...register('password', { required: true })} placeholder="请输入密码" />
        {errors.password && <p className="pt-2 pl-3 text-base text-errorColor-400">密码不能为空</p>}
      </div>
    </fieldset>
