...
const OrgListCard: FC<OrgListProps> = (props) => {
  return (
    <div
      className="flex flex-col max-w-sm mb-3 ml-2 mr-2 bg-white rounded-lg shadow-md h-36"
      onClick={() => {
        Router.push({
          pathname: "/searchLessonList",
          query: { item: JSON.stringify(props.item) },
        });
      }}
    >
      <img
        className="h-20 rounded-t-lg cursor-pointer"
        src={props.eduImage}
        alt=""
      />
      <div className="flex flex-col justify-center px-2 pt-1">
        <div className="flex items-center justify-center">
          <div className="flex flex-auto text-xs font-bold text-gray-900 truncate cursor-pointer justify-self-start">
            {props.eduName}
          </div>
          <div className="flex flex-auto justify-self-end">
            <svg
              className="w-3 h-3 m-1 text-yellow-500 "
              fill="orange"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
            ...
            <svg
              className="w-3 h-3 mt-1 mr-1 text-yellow-500 "
              fill="orange"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
            ...
            <svg
              className="w-3 h-3 mt-1 text-yellow-500 "
              fill="orange"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
        ...
        <p className="pt-1 text-xs text-gray-400 truncate">
          <span>地址：</span>
          {props.eduAddress}
        </p>
        <p className="text-xs text-gray-400 truncate">
          <span>电话：</span>
          {props.eduContactPhone}
        </p>
      </div>
    </div>
  );
          ...
          <div className="bg-white">
            ...
            <div className="grid items-center grid-cols-6 mt-3">
              <div className="col-span-5 text-sm font-bold text-gray-900">
                <svg
                  className="inline w-5 h-5 ml-1 text-primary-600"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  strokeWidth="4"
              ...
              <div className="flex mb-1 mr-3 text-xs text-gray-400 justify-self-end">
                <a href="http://www.w3school.com.cn">
                  <div>
                    更多
                    <svg
                      className="inline w-3 h-3 mb-1 text-gray-400"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      strokeWidth="2"
            ...
            <div className="grid py-2 sm1:grid-cols-2 sm2:grid-cols-2 sm3:grid-cols-2">
              {orgListDemo.map((item, index) => {
                return (
                  <OrgListCard
                    key={index}
                    eduImage={item.eduImage}
                    eduName={item.eduName}
                    eduAddress={item.eduAddress}
                    eduContactPhone={item.eduContactPhone}
                  />
                );
              })}
            </div>
