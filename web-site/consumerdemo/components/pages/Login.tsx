...
const MyLoginTabs = () => {
  ...
  const onSubmit = (loginType: string) => (data: any) => {
    console.log("login");
    ...
    <Tab.Group selectedIndex={selectedIndex} onChange={setSelectedIndex}>
          ...
          {({ selected }) => {
            return (
              <button
                ...
                className={
                  selected
                    ? " px-2 pt-2 mx-2 mt-4 font-bold  text-primary-600 focus:outline-none border"
                    : "px-2 py-2 mx-2 my-4  text-gray-800 font-bold "
                }
              >
                验证码登录
              </button>
          ...
          {({ selected }) => {
            return (
              <button
                className={
                  selected
                    ? " px-2 pt-2 mx-2 mt-4 font-bold  text-primary-600 focus:outline-none"
                    : "px-2 py-2 mx-2 my-4  text-gray-800 font-bold "
                }
              >
                账号登录
      ...
      <Tab.Panels>
        ...
        <Tab.Panel>
          <form
            id="1"
            onSubmit={handleSubmitVc(onSubmit("verfiyCode"), onError)}
          >
            <div className="flex px-2 mx-2 my-2 text-sm">
              <div className="pr-4">
                <svg
                  className="inline w-6 h-6 text-primary-600"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  strokeWidth="2"
            ...
            <div className="flex px-2 mx-2 my-5 text-sm">
              ...
              <input
                {...registerVc("verifyCode")}
                className="inline w-full border-b focus:outline-none"
                placeholder="请输入验证码"
              />
              <input
                className="px-4 py-1 border justify-self-end rounded-3xl text-primary-500 border-primary-500"
                type="button"
                value="获取验证码"
              />
            </div>
  ...
  return (
    <div>
      <div className="flex mt-12 text-base">
        <input
          className="w-full h-10 py-2 mx-6 font-bold tracking-widest text-white shadow-md rounded-3xl bg-primary-600 bg-grimary-600 shadow-primary-600 focus:bg-primary-700"
          onClick={console.log}
          type="submit"
          value="登录"
        />
      </div>
      <p className="pt-8 text-sm text-center text-gray-500">
        登录即同意资金监管平台
