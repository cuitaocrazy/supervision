...
import { searchRefundContractURL } from "../../const/const";
...
const LessonListCard: FC<OrderListProps> = (props) => {
  ...
  const refreshContractDetail = useCallback(
    (contract: Contract) => {
    ...
    <div className="pb-3 mx-3 mt-2 bg-white rounded-lg shadow-md">
      <div className="flex pb-1 mx-2 mb-2 rounde-xl">
        ...
        <div className="mt-3 ml-3">
          ...
          <div className="flex mt-1">
            <div className="text-sm text-gray-400 ">总课时：</div>
            <div className="text-sm text-gray-700">
              {props.lessonTotalQuantity}
              <span>/</span>
            </div>
            <div className="pl-1 text-sm text-orange-400">
              {props.lessonCompletedQuantity}
            </div>
          </div>
        </div>
      </div>
      <div className="grid grid-cols-3 gap-2 text-xs text-white justify-items-center">
        <div
          className="px-6 py-1 shadow-md rounded-3xl bg-secondary-300 shadow-secondary-300"
          onClick={() => {
            Router.push({
              pathname: "./myApplyComp",
            ...
            {orderList.map((item, index) => {
              return (
                <LessonListCard
                  ...
                  consumerStuName={item.consumerStuName}
                  lessonTotalQuantity={item.lessonTotalQuantity}
                  lessonCompletedQuantity={item.lessonAccumulationQuantity}
                  item={item}
                />
              );
            })}
