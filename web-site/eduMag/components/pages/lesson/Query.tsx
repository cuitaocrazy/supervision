...
const LessonQuery: React.FC = () => {
  ...
  const ListEntry = ({ lesson, key, ...props }: { lesson: Lesson, key: any }) => (
    <ul key={key} className="grid items-center grid-cols-8 gap-10 text-gray-600 border justify-items-center even:bg-primary-100 odd:bg-white ">
      <li className='flex items-center justify-center leading-10'>{lesson.lessonName}</li>
      <li className='flex items-center justify-center leading-10'>{lesson.lessonTotalPrice}</li>
      <li className='flex items-center justify-center leading-10'>{Number(lesson.lessonTotalPrice) / 100}</li>
      <li className='flex items-center justify-center leading-10'>{lesson.lessonType}</li>
      <li className='flex items-center justify-center leading-10'>{lesson.lessonStartDate}</li>
      <li className='flex items-center justify-center leading-10'>{lesson.lessonEndDate}</li>
      <li className='flex items-center justify-center leading-10'>{getStatus(lesson.lessonStatus)}</li>
      <li className='flex items-center justify-center leading-10'>
        <div className='flex gap-2 '>
          <button className='p-1 rounded-md text-primary-600' onClick={onDetail(lesson)}>查看详情</button>
          {lesson.lessonStatus === 'on' ? <button className='p-1 text-red-500 rounded-md' onClick={() => { setCancelLesson(lesson); setIsCancelModalOpen(true) }}>下架</button> : <></>}
          {lesson.lessonStatus === 'pending' ? <button className='p-1 rounded-md text-sky-600' onClick={onEdit(lesson)}>编辑</button> : <></>}
        </div>
      </li>
    </ul>
  );
    ...
    <IonContent>
      <div className='relative w-screen px-4 bg-gray-100'>
        <div className='flex pt-2 my-2 text-gray-800'>
          ...
          <div>
            <span className='pr-1 text-gray-600'>课程管理</span>/<span className='pl-1 text-primary-500'>课程列表</span>
          </div>
        </div>
          ...
          <table className=''>
            <thead>
              <ul className='grid items-center h-10 grid-cols-8 gap-10 font-bold text-gray-700 bg-white rounded-lg justify-items-center'>
                ...
                <li className='flex items-center justify-center'>总课时（个）</li>
